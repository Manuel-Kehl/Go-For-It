 ...
 */
class GOFI.Plugins.TodoTXT.SettingsManager {
    ...
    public SettingsManager.load_from_key_file () {
        ...
        if (!FileUtils.test (Utils.config_file, FileTest.EXISTS)) {
            ...
            try {
                stdout.printf ("%s\n", Utils.config_file);
                key_file.load_from_file (Utils.config_file,
                   KeyFileFlags.KEEP_COMMENTS | KeyFileFlags.KEEP_TRANSLATIONS);
            } catch (Error e) {
                stderr.printf("Reading %s failed", Utils.config_file);
                error ("%s", e.message);
            }
        }
    }
     ...
     * "standard locations", which are defined in TEST_DIRS in Constants.vala.
     */
    ...
    private void write_key_file () throws FileError {
        key_file.save_to_file (Utils.config_file);
    }
}
